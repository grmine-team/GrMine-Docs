...
| `gm-accordion-trigger` | 折叠项触发按钮 |
| `gm-accordion-icon` | 展开指示图标（展开时旋转 180°） |
| `gm-accordion-content` | 折叠项内容 |
| `gm-accordion-item-open` | 展开状态 |

## 注意事项

- 展开状态通过添加 `gm-accordion-item-open` 类名控制
- 内容隐藏可通过 `[hidden]` 属性实现
- 需配合 JavaScript 控制展开/折叠逻辑

## 暗色模式

暗色模式下折叠项背景使用深色，触发按钮悬停态使用更深的灰色，展开指示箭头颜色自动适配。
